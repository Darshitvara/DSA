public class DoublyLinkedList {
    public static void main(String[] args) {
        ...
        reversePrintLL(node5); // Print the linked list in reverse order
    }
    ...
    public static int searchLL(Node head, int value) {
        ...
        while(current != null){
            ...
            if(current.data == value){
                return counter; // Return the position if value is found
            }
            current = current.next; // Move to the next node
        }
    ...
    public static void reversePrintLL(Node tail) {
        Node current = tail;
        while(current != null){
            System.out.print(current.data + " ");
